...
/// a 257-entry luma curve, coarse saturation/hue sliders, and sharpening. The
/// luma curve is fit from neutral gray samples, while saturation/hue and the
...
///
/// The file uses a neutral base Picture Control header and replaces the embedded
/// 257-entry user-defined curve with the optimizer result. Coarse slider bytes
/// are filled with best-effort values but the luma LUT is the important payload.
...
fn build_picture_control(
    ...
    let saturation = estimate_saturation_slider(&samples, adjustments);
    let hue = estimate_hue_slider(&samples, adjustments);
    ...
    let brightness = (adjustments.exposure * 2.0 + adjustments.whites / 50.0
        ...
        .round()
        .clamp(-3.0, 3.0) as i8;
    let report = estimate_report(&curve, &samples, saturation, hue, sharpening);
...
fn estimate_report(
    ...
    for sample in samples {
        ...
        let approx = approximate_rgb(sample.input, curve, saturation, hue);
...
fn neutral_ncp_template() -> [u8; NCP_SIZE] {
    ...
    bytes[0x24..0x48].copy_from_slice(&[
        0x03, 0xc2, 0x00, 0xff, 0x82, 0x01, 0x01, 0x80, 0x80, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00,
        0x02, 0x00, 0x00, 0x02, 0x42, 0x49, 0x30, 0x00, 0xff, 0x00, 0xff, 0x01, 0x00, 0x02, 0x00,
