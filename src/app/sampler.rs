...
use crate::app::profile::profile_from_xmp;
...
struct SamplerProgress<'a> {
    profile: &'a ProgressBar,
    started: Instant,
}
...
pub(crate) fn run_sampler(args: SamplerArgs) -> Result<()> {
    ...
    let profiles = collect_xmp_profiles(&args.profiles_root)?;
    if profiles.is_empty() {
        bail!("no XMP files found under {}", args.profiles_root.display());
    }
    ...
    let profile_progress = multi.add(ProgressBar::new(5));
    profile_progress.set_style(profile_progress_style());
    profile_progress.set_message(raw_engine_step(args.raw_engine));
    ...
    profile_progress.set_position(5);
    profile_progress.set_message("raw develop done");

    let started = Instant::now();
    let mut thumbs = Vec::new();
    let mut skipped = 0usize;
    for (index, profile) in profiles.iter().enumerate() {
        sampler.set_position(index as u64);
        sampler.set_message(
            profile
                .file_name()
                .and_then(|s| s.to_str())
        ...
        match render_profile_thumbnail(
            &args,
            temp_dir.path(),
            &base_tiff,
            profile,
    ...
    profile_progress.set_position(0);
    profile_progress.set_message("montage");
    run_montage(&args.montage, &args.output, &thumbs, args.jpg_quality)?;
    profile_progress.finish_and_clear();
...
fn render_profile_thumbnail(
    ...
    let resolved = profile_from_xmp(profile, args.hald_level, &args.profiles_root, &profile_temp)
    ...
    let label = profile
        .strip_prefix(&args.profiles_root)
        .unwrap_or(profile)
        .display()
        .to_string();
...
fn sampler_step(progress: &SamplerProgress<'_>, position: u64, step: &str) {
