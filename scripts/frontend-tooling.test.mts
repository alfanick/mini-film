...
import { formatStagedAssets } from "./format-staged-assets.mts";
import { isRecord } from "./tooling.mts";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const projects: readonly [string, string, readonly string[]][] = [
  ...
  ["scripts/build-review.mts", "scripts/tsconfig.json", ["node"]],
];

// Each source category must resolve a checked-in configuration, not an inferred project.
void test("TypeScript discovers browser, test, and Playwright projects with strict options", () => {
  for (const [relative, expectedConfig, types] of projects) {
    const file = join(root, relative);
    const configPath = ts.findConfigFile(dirname(file), (path) => ts.sys.fileExists(path));
    ...
    const configuration: unknown = config.config;
    assert.ok(isRecord(configuration));
    const parsed = ts.parseJsonConfigFileContent(configuration, ts.sys, dirname(configPath));
...
void test("type-aware linting rejects unsafe values, h views, suppressions, and unused declarations", async () => {
  const eslint = new ESLint({ cwd: root });
  for (const [relative] of projects) {
    const config: unknown = await eslint.calculateConfigForFile(join(root, relative));
    assert.ok(isRecord(config));
    const languageOptions = config["languageOptions"];
    const linterOptions = config["linterOptions"];
    const rules = config["rules"];
    assert.ok(isRecord(languageOptions) && isRecord(linterOptions) && isRecord(rules));
    const parserOptions = languageOptions["parserOptions"];
    assert.ok(isRecord(parserOptions));
    assert.equal(parserOptions["projectService"], true);
    assert.equal(linterOptions["noInlineConfig"], true);
    for (const rule of [
      ...
      assert.ok(Array.isArray(setting));
      assert.equal(setting[0], 2, `${relative}: ${rule} is an error`);
    }
  }
});
...
void test("pre-commit formatting touches staged paths but preserves index and unstaged files", async (context) => {
