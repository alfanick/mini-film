// Generate editor and build-time contracts from Rust-owned wire schemas. Cargo
// supplies fresh schemas; standalone frontend builds use the checked-in mirror.
import assert from "node:assert/strict";
import { mkdir, mkdtemp, readFile, readdir, rm, writeFile } from "node:fs/promises";
import { createRequire } from "node:module";
import { tmpdir } from "node:os";
import { dirname, join, resolve } from "node:path";
import { spawnSync } from "node:child_process";
import { fileURLToPath } from "node:url";
import type { Node, SourceFile, TransformerFactory, VisitResult } from "typescript";
import { errorMessage, isRecord, readJson, required } from "./tooling.mts";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
export const contractInputs = [
  "requests.schema.json",
  "responses.schema.json",
  "operations.json",
  "fixtures.json",
] as const;
const banner = "/** Generated from Rust wire contracts; regenerate with npm run contracts:generate. */";

type SchemaDocument = Record<string, unknown>;
type SchemaValidator = import("ajv").Ajv;

/** The used compiler surface avoids an upstream ref-parser declaration incompatible with exact optional types. */
interface SchemaCompiler {
  compile: (
    schema: SchemaDocument,
    name: string,
    options: {
      bannerComment: string;
      unknownAny: boolean;
      additionalProperties: boolean;
      strictIndexSignatures: boolean;
      enableConstEnums: boolean;
      style: { printWidth: number; trailingComma: "all" };
    },
  ) => Promise<string>;
}

/** Schema validation makes dictionary properties safe to enumerate without trusting parsed JSON. */
function properties(schema: SchemaDocument): Record<string, unknown> {
  const value = schema["properties"];
  if (!isRecord(value)) throw new Error("Contract schema requires a properties dictionary");
  return value;
}

/** Rust declares the exact representation of each route identity. */
interface OperationParameter {
  name: string;
  kind: "number" | "string";
}

/** Validated operation metadata feeds both the typed client and test-only request decoders. */
interface Operation {
  name: string;
  method: string;
  path: string;
  request: string | null;
  response: string;
  allow_empty_request: boolean;
  transport: "http" | "sse";
  parameters: readonly OperationParameter[];
}

/** Parse operation metadata without broadening named identities or accepting mismatched path templates. */
export function parseOperations(value: unknown): readonly Operation[] {
  if (!Array.isArray(value)) throw new Error("Operation manifest must be an array");
  const names = new Set<string>();
  return value.map((entry: unknown): Operation => {
    if (!isRecord(entry)) throw new Error("Operation metadata must be an object");
    const name = entry["name"];
    const method = entry["method"];
    const path = entry["path"];
    const request = entry["request"];
    const response = entry["response"];
    const allowEmpty = entry["allow_empty_request"];
    const transport = entry["transport"];
    const parameters = entry["parameters"];
    if (
      typeof name !== "string" ||
      !/^[a-z][a-z_]*$/.test(name) ||
      typeof method !== "string" ||
      !["GET", "POST", "PATCH", "DELETE"].includes(method) ||
      typeof path !== "string" ||
      (request !== null && typeof request !== "string") ||
      typeof response !== "string" ||
      typeof allowEmpty !== "boolean" ||
      (transport !== "http" && transport !== "sse") ||
      !Array.isArray(parameters)
    )
      throw new Error("Invalid operation metadata");
    if (names.has(name)) throw new Error(`Duplicate operation name: ${name}`);
    names.add(name);
    const declared = parameters.map((parameter: unknown): OperationParameter => {
      if (!isRecord(parameter)) throw new Error(`Invalid parameter in ${name}`);
      const parameterName = parameter["name"];
      const kind = parameter["kind"];
      if (typeof parameterName !== "string" || (kind !== "number" && kind !== "string")) {
        throw new Error(`Invalid parameter in ${name}`);
      }
      return { name: parameterName, kind };
    });
    const placeholders = [...path.matchAll(/\{([^{}]+)\}/g)].map((match) => required(match[1]));
    const parameterNames = declared.map((parameter) => parameter.name);
    if (
      new Set(parameterNames).size !== parameterNames.length ||
      new Set(placeholders).size !== placeholders.length ||
      JSON.stringify([...placeholders].sort()) !== JSON.stringify([...parameterNames].sort())
    )
      throw new Error(`Path parameters do not match the template for ${name}`);
    return { name, method, path, request, response, allow_empty_request: allowEmpty, transport, parameters: declared };
  });
}

/** Validate schema objects before passing their typed shape into code-generation libraries. */
function schemaDocument(value: unknown, validator: SchemaValidator): SchemaDocument {
  if (!isRecord(value) || validator.validateSchema(value) !== true) throw new Error("Invalid JSON Schema document");
  return value;
}

/** Keep generated identifiers stable, including endpoint names containing separators. */
function identifier(value: string): string {
  return value.replace(/(^|[_-])([a-z])/g, (_match: string, _separator: string, letter: string) =>
    letter.toUpperCase(),
  );
}

/** Required lists already describe optionality; defaults otherwise create redundant enum/string intersections. */
function declarationSchema(value: SchemaDocument, validator: SchemaValidator): SchemaDocument {
  return schemaDocument(withoutAnnotations(value, new Set(["default"])), validator);
}

/** Keep documentation in editor schemas/types, not in runtime tables used only for validation constraints. */
function validationSchema(value: SchemaDocument, validator: SchemaValidator): SchemaDocument {
  return schemaDocument(
    withoutAnnotations(value, new Set(["title", "description", "default", "examples", "$comment"])),
    validator,
  );
}

/** Compact opaque Ajv failure details while preserving boolean validation, counters, arrays, and schema constants. */
export function compactValidationErrors(source: string, ts: typeof import("typescript")): string {
  const file = ts.createSourceFile("validators.mjs", source, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS);
  let changed = 0;
  const transform: TransformerFactory<SourceFile> = (context) => {
    /** Recognize only Ajv's generated error temporaries and validator .errors assignments. */
    function isErrorContext(node: Node): boolean {
      let owner: Node | undefined = node.parent;
      while (owner !== undefined && !ts.isFunctionDeclaration(owner)) owner = owner.parent;
      if (
        owner === undefined ||
        !ts.isFunctionDeclaration(owner) ||
        !owner.name ||
        !/^validate\d+$/.test(owner.name.text)
      )
        return false;
      const parent = node.parent;
      if (ts.isVariableDeclaration(parent) && ts.isIdentifier(parent.name) && /^err\d+$/.test(parent.name.text)) {
        return true;
      }
      if (!ts.isArrayLiteralExpression(parent) || !ts.isBinaryExpression(parent.parent)) return false;
      const assignment = parent.parent;
      return (
        assignment.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
        assignment.right === parent &&
        ts.isPropertyAccessExpression(assignment.left) &&
        assignment.left.name.text === "errors" &&
        ts.isIdentifier(assignment.left.expression) &&
        /^validate\d+$/.test(assignment.left.expression.text)
      );
    }
    /** Error arrays and counters remain unchanged; an unexpected generator shape fails instead of guessing. */
    function visit(node: Node): VisitResult<Node> {
      if (!ts.isObjectLiteralExpression(node) || !isErrorContext(node)) return ts.visitEachChild(node, visit, context);
      const names = node.properties.map((property) => {
        const named = ts.isPropertyAssignment(property) || ts.isShorthandPropertyAssignment(property);
        return named && ts.isIdentifier(property.name) ? property.name.text : "";
      });
      // Ajv already emits empty errors inside negative/composite branches.
      if (names.length === 0) return node;
      if (
        names.length !== 4 ||
        !["instancePath", "schemaPath", "keyword", "params"].every((name) => names.includes(name))
      ) {
        throw new Error(`Unexpected pinned Ajv error shape (${names.join(",")}); review validator compaction`);
      }
      changed += 1;
      return ts.factory.createObjectLiteralExpression();
    }
    return (node): SourceFile => required(ts.visitNode(node, visit, ts.isSourceFile));
  };
  const result = ts.transform(file, [transform]);
  try {
    if (changed === 0) throw new Error("Pinned Ajv emitted no recognized error payloads; review validator compaction");
    return ts.createPrinter().printFile(required(result.transformed[0]));
  } finally {
    result.dispose();
  }
}

/** Traverse schema positions, preserving properties literally named title, description, default, or examples. */
function withoutAnnotations(value: unknown, annotations: ReadonlySet<string>): unknown {
  if (Array.isArray(value)) return value.map((child: unknown): unknown => withoutAnnotations(child, annotations));
  if (!isRecord(value)) return value;
  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => !annotations.has(key))
      .map(([key, child]) => {
        if (
          ["properties", "definitions", "$defs", "patternProperties", "dependencies", "dependentSchemas"].includes(key)
        ) {
          if (!isRecord(child)) throw new Error(`Schema ${key} must be an object`);
          return [
            key,
            Object.fromEntries(
              Object.entries(child).map(([name, schema]) => [name, withoutAnnotations(schema, annotations)]),
            ),
          ];
        }
        if (key === "enum" || key === "const") return [key, child];
        return [key, withoutAnnotations(child, annotations)];
      }),
  );
}

/** Generation uses Rust-exported schemas in Cargo and their tracked mirrors in standalone developer commands. */
export interface GenerateOptions {
  schemaDir: string;
  outputDir: string;
  dependenciesDir?: string;
  runtimeOnly?: boolean;
}

/** Generate TypeScript, named operations and ahead-of-time validators without runtime schema loading. */
export async function generateContracts({
  schemaDir,
  outputDir,
  dependenciesDir = root,
  runtimeOnly = false,
}: GenerateOptions): Promise<string[]> {
  const require = createRequire(join(dependenciesDir, "package.json"));
  // Cargo's isolated install supplies these packages; their published declarations type Node's dynamic loader.
  const { compile } = require("json-schema-to-typescript") as SchemaCompiler;
  const { Ajv } = require("ajv") as typeof import("ajv");
  const { default: standalone } = require("ajv/dist/standalone") as typeof import("ajv/dist/standalone/index.js");
  const prettier = require("prettier") as typeof import("prettier");
  const ts = require("typescript") as typeof import("typescript");
  const inputs = await Promise.all(contractInputs.map((file) => readFile(join(schemaDir, file), "utf8")));
  const ajv = new Ajv({
    strict: true,
    strictRequired: false,
    allErrors: false,
    // Inline tiny primitive references, but share larger image/profile validators
    // across HTTP, SSE, full-state and patch decoders to keep the bundle bounded.
    inlineRefs: 3,
    loopRequired: 5,
    messages: false,
    coerceTypes: false,
    useDefaults: false,
    removeAdditional: false,
    code: { source: true, esm: true, optimize: 2 },
  });
  // Schemars formats annotate Rust numeric storage, while JSON constraints still
  // enforce integer/range validity. They are not string-format validators.
  for (const format of [
    "uint8",
    "uint16",
    "uint32",
    "uint64",
    "uint",
    "int8",
    "int16",
    "int32",
    "int64",
    "int",
    "float",
    "double",
  ]) {
    ajv.addFormat(format, true);
  }
  const requests = schemaDocument(await readJson(join(schemaDir, "requests.schema.json")), ajv);
  const responses = schemaDocument(await readJson(join(schemaDir, "responses.schema.json")), ajv);
  const operations = parseOperations(await readJson(join(schemaDir, "operations.json")));
  const catalogs = { requests, responses };
  const exports: Record<string, string> = {};
  const declarations = [
    banner,
    'import type { RequestContracts } from "./requests";',
    'import type { ResponseContracts } from "./responses";',
  ];
  const generated = new Map<string, string>();
  for (const [direction, schema] of Object.entries(catalogs)) {
    const catalogName = direction === "requests" ? "RequestContracts" : "ResponseContracts";
    const base = `https://mini-film.invalid/contracts/${direction}`;
    ajv.addSchema({ ...validationSchema(schema, ajv), $id: base }, base);
    generated.set(
      `${direction}.ts`,
      await compile(declarationSchema(schema, ajv), catalogName, {
        bannerComment: banner,
        unknownAny: true,
        additionalProperties: false,
        strictIndexSignatures: true,
        enableConstEnums: false,
        style: { printWidth: 120, trailingComma: "all" },
      }),
    );
    for (const name of Object.keys(properties(schema)).sort()) {
      const validator = `validate${direction === "requests" ? "Request" : "Response"}${identifier(name)}`;
      exports[validator] = `${base}#/properties/${name}`;
      declarations.push(
        `/** Validate the ${name} ${direction} boundary before consuming unknown JSON. */`,
        `export function ${validator}(value: unknown): value is ${catalogName}[${JSON.stringify(name)}];`,
      );
    }
  }
  generated.set("validators.mjs", `${banner}\n${compactValidationErrors(standalone(ajv, exports), ts)}\n`);
  generated.set("validators.d.mts", declarations.join("\n"));
  const operationLines = [
    banner,
    'import type { RequestContracts } from "./requests";',
    'import type { ResponseContracts } from "./responses";',
    'import * as validators from "./validators.mjs";',
    "/** Endpoint bindings originate in Rust, preventing caller-selected response assertions. */",
    "export interface OperationContracts {",
  ];
  for (const operation of operations) {
    if (operation.request !== null && !Object.hasOwn(properties(requests), operation.request)) {
      throw new Error(`Missing request schema for ${operation.name}`);
    }
    if (!Object.hasOwn(properties(responses), operation.response)) {
      throw new Error(`Missing response schema for ${operation.name}`);
    }
    const request = operation.request === null ? "undefined" : `RequestContracts[${JSON.stringify(operation.request)}]`;
    const fields = operation.parameters.map((parameter) => `${JSON.stringify(parameter.name)}: ${parameter.kind}`);
    const parameters = operation.parameters.length === 0 ? "never" : `{ ${fields.join("; ")} }`;
    operationLines.push(
      `${JSON.stringify(operation.name)}: { request: ${request};`,
      `response: ResponseContracts[${JSON.stringify(operation.response)}]; parameters: ${parameters} };`,
    );
  }
  operationLines.push(
    "}",
    "/** Concrete decoders and route templates for every supported JSON operation. */",
    "export const operations = {",
  );
  for (const operation of operations) {
    operationLines.push(
      `${JSON.stringify(operation.name)}: {`,
      `method: ${JSON.stringify(operation.method)}, path: ${JSON.stringify(operation.path)},`,
      `allowEmptyRequest: ${Boolean(operation.allow_empty_request)},`,
      `decode: validators.validateResponse${identifier(operation.response)},`,
      `hasRequest: ${operation.request !== null},`,
      "},",
    );
  }
  operationLines.push("} as const;");
  generated.set("operations.ts", operationLines.join("\n"));
  const client = [
    banner,
    'import { createOperation, type OperationOptions } from "../core/transport";',
    'import { operations, type OperationContracts } from "./operations";',
    "/** Named API methods prevent callers from inventing response types or forgetting route identities. */",
    "export const reviewApi = {",
  ];
  for (const operation of operations.filter((operation) => operation.transport !== "sse")) {
    const key = JSON.stringify(operation.name);
    const request = `OperationContracts[${key}]["request"]${operation.allow_empty_request ? " | undefined" : ""}`;
    const response = `OperationContracts[${key}]["response"]`;
    const params = `OperationContracts[${key}]["parameters"]`;
    // The variable annotation documents each public function as well as its implementation's generic binding.
    client.push(`${key}: createOperation<${request}, ${response}, ${params}>(operations[${key}]),`);
  }
  client.push(
    "} as const;",
    "/** Derive exact call options for helper functions composing an existing operation. */",
    "export type ReviewOperationOptions<Request, Params> = OperationOptions<Request, Params>;",
  );
  generated.set("client.ts", client.join("\n"));
  const requestDecoders = [
    banner,
    'import type { OperationContracts } from "./operations";',
    'import { operations } from "./operations";',
    'import * as validators from "./validators.mjs";',
    "/** Correlate operation identity with its exact body, including deliberately empty creation requests. */",
    "export type OperationRequest<Name extends keyof OperationContracts> = OperationContracts[Name]['request'] |",
    "  (typeof operations[Name]['allowEmptyRequest'] extends true ? undefined : never);",
    "/** Test-only decoders verify the outgoing body without adding request validators to the browser entry point. */",
    "export const requestDecoders: {",
    "  [Name in keyof OperationContracts]: (value: unknown) => OperationRequest<Name>",
    "} = {",
  ];
  for (const operation of operations) {
    const key = JSON.stringify(operation.name);
    requestDecoders.push(
      `/** Decode the ${operation.name} body using its Rust-owned request contract. */`,
      `${key}: (value: unknown): OperationRequest<${key}> => {`,
    );
    if (operation.request === null || operation.allow_empty_request) {
      requestDecoders.push("if (value === undefined) return undefined;");
    }
    if (operation.request !== null) {
      requestDecoders.push(`if (validators.validateRequest${identifier(operation.request)}(value)) return value;`);
    }
    requestDecoders.push(`throw new Error("Invalid ${operation.name} request body");`, "},");
  }
  requestDecoders.push("};");
  requestDecoders.push(
    "/** Preserve the operation name and decoded body as one discriminated recording. */",
    "export type RecordedOperation = {",
    "[Name in keyof OperationContracts]: {name: Name; body: OperationRequest<Name>}",
    "}[keyof OperationContracts];",
    "/** Decode a matched HTTP route without widening the relationship between its name and body. */",
    "export function decodeOperationRequest(name: keyof OperationContracts, body: unknown): RecordedOperation {",
    "switch (name) {",
  );
  for (const operation of operations) {
    const key = JSON.stringify(operation.name);
    requestDecoders.push(`case ${key}: return {name: ${key}, body: requestDecoders[${key}](body)};`);
  }
  requestDecoders.push("}", "}");
  generated.set("request-decoders.ts", requestDecoders.join("\n"));
  for (const [index, file] of contractInputs.entries()) generated.set(file, required(inputs[index]));
  await mkdir(outputDir, { recursive: true });
  const outputs = [...generated].filter(([file]) => !runtimeOnly || file === "validators.mjs");
  for (const [file, source] of outputs) {
    // Standalone validator JS is machine output, not hand-maintained application
    // code; its declaration and the consuming TS remain fully type-checked.
    const contents = await prettier.format(source, {
      parser: file.endsWith(".json") ? "json" : file.endsWith(".mjs") ? "babel" : "typescript",
      printWidth: 120,
      trailingComma: "all",
    });
    await writeFile(join(outputDir, file), contents);
  }
  return outputs.map(([file]) => file).sort();
}

/** Regenerate ignored runtime output, or export Rust schemas to update/verify the tracked editor mirror. */
async function main(mode: string | undefined): Promise<void> {
  const mirror = join(root, "frontend/review/generated");
  if (mode === "--runtime") {
    await generateContracts({ schemaDir: mirror, outputDir: mirror, runtimeOnly: true });
    return;
  }
  if (mode !== "--generate" && mode !== "--check") throw new Error("Expected --generate, --check, or --runtime");
  const temporary = await mkdtemp(join(tmpdir(), "mini-film-contracts-"));
  try {
    const schemas = join(temporary, "schemas");
    const exported = spawnSync(
      "cargo",
      ["run", "--locked", "-p", "review-contract-export", "--", "--out-dir", schemas],
      {
        cwd: root,
        stdio: "inherit",
        windowsHide: true,
      },
    );
    if (exported.error || exported.status !== 0) throw new Error("Rust review contract export failed");
    const destination = mode === "--generate" ? mirror : join(temporary, "generated");
    const files = await generateContracts({ schemaDir: schemas, outputDir: destination });
    if (mode === "--check") {
      const trackedFiles = files.filter((file) => file !== "validators.mjs");
      assert.deepEqual(
        (await readdir(mirror)).filter((file) => file !== "validators.mjs").sort(),
        trackedFiles,
        "Generated contract file list is stale",
      );
      for (const file of trackedFiles) {
        assert.equal(
          await readFile(join(mirror, file), "utf8"),
          await readFile(join(destination, file), "utf8"),
          `${file} is stale; run npm run contracts:generate`,
        );
      }
    }
  } finally {
    await rm(temporary, { recursive: true, force: true });
  }
}

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main(process.argv[2]).catch((error: unknown) => {
    console.error(errorMessage(error));
    process.exitCode = 1;
  });
}
