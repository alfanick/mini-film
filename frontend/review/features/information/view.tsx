...
export function ProfileInfoOverlay(): ComponentChildren {
  ...
  const pp3Text =
    pp3State?.status === "failed"
      ? pp3State.error
      : pp3State?.status === "ready"
        ? pp3State.text || "Loading..."
        : "Loading...";
  return (
    <section class={"profile-info-card"}>
      <header class={"profile-info-header"}>
        <div>
          <h3 id="profile-info-title">{"Profile info"}</h3>
      ...
      {pp3Url && image ? (
        ...
        >
          ...
          <pre class={`profile-info-pp3 ${pp3State?.status === "failed" ? "profile-info-pp3-error" : ""}`}>
            {pp3Text}
          </pre>
        </details>
      ) : null}
...
export function CommandInvocationOverlay(): ComponentChildren {
  ...
  return (
    <section class={"command-invocation-card"}>
      <header class={"command-invocation-header"}>
        <div>
          <h3 id="command-invocation-title">{"Command invocation"}</h3>
