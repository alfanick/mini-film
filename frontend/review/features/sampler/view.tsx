    ...
    <section class={"sampler-card"}>
      ...
      {job ? (
        <div class={"sampler-progress"}>
          <progress max={progressMax} value={progressValue} aria-label="Sampler rendering progress" />
          <span class={job.error ? "error" : ""} role="status" aria-live="polite">
            {job.error || samplerStatusText(job)}
          </span>
        </div>
      ) : null}
      {state.samplerError ? (
        <div class={"sampler-error"} role="alert">
          {state.samplerError}
        </div>
      ) : null}
      {job?.source_url ? (
