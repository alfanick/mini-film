...
import { requestJson, reviewUrl } from "../core/api";
...
import { buildSamplerHierarchy, type SamplerSectionData } from "./sampler-helpers";
import { errorMessage, isAbortError } from "./common";
...
export function useSampler(): SamplerActions {
  ...
  const openSampler = useCallback(async (): Promise<void> => {
    ...
    try {
      const job = await requestJson<SamplerJob>("api/sampler/jobs", "POST", { image_id: image.id });
  ...
  useEffect(() => {
    ...
    const poll = async (): Promise<void> => {
      try {
        const job = await requestJson<SamplerJob>(`api/sampler/jobs/${jobId}`, "GET", undefined, controller.signal);
  ...
  const updateSamplerSelection = useCallback(
    async (entry: SamplerEntry, scope: DiffusionScope, enabled: boolean): Promise<void> => {
      ...
      try {
        const job = await requestJson<SamplerJob>(`api/sampler/jobs/${jobId}/profiles/${entry.key}`, "POST", {
          scope,
          enabled,
        });
        if (generation.current !== request) return;
  ...
  useEffect(() => {
    ...
    const sendPriority = (): void => {
      ...
      timer = setTimeout(() => {
        ...
        void fetch(reviewUrl(`api/sampler/jobs/${job.id}/priority`), {
          method: "POST",
