...
import { requestJson, errorMessage } from "../core/api";
...
import { numberOrNull, splitPublishTags } from "./publish-helpers";
...
export function usePublish(session: ToolSessionActions): PublishActions {
  ...
  const submitPublish = async (): Promise<void> => {
    ...
    setError("");
    try {
      const message = await requestJson<ReviewStateMessage>("api/publish", "POST", publishBody(publishForm));
      if (message) session.applyMessage(message);
    } catch (error) {
  ...
  const publishStats = { pictures: 0, outputs: 0 };
  for (const image of state.data?.images || []) {
    if (
      ...
      continue;
    publishStats.pictures += 1;
    publishStats.outputs += isDirectCompressedImage(image)
      ? 1
  ...
  const publishJob = jobs.length ? jobs[jobs.length - 1] : null;
  return {
    publishOpen,
    publishForm,
    publishSubmitting,
