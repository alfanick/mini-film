...
import { requestJson, errorMessage, type HttpMethod } from "../core/api";
import type {
...
/** The distinct body shapes accepted by project creation/update, preview, and final-render operations. */
type PanoramaRequestBody =
  | { image_ids: number[]; matching_mode: PanoramaMatching; name?: string }
  | { name: string; projection: PanoramaProjection };
...
export function usePanorama(session: ToolSessionActions): PanoramaActions {
  ...
  const request = async (path: string, method: HttpMethod, body: PanoramaRequestBody): Promise<void> => {
    const data = await requestJson<ReviewStateMessage>(path, method, body);
    if (data) session.applyMessage(data);
  };
  ...
  const generatePanoramaPreviews = async (): Promise<void> => {
    ...
    try {
      ...
      if (projectId === null) {
        ...
        await request("api/panoramas", "POST", body);
      ...
      } else await request(`api/panoramas/${projectId}`, "PATCH", body);
      await request(`api/panoramas/${projectId}/previews`, "POST", {
        image_ids: current.panoramaImageIds,
        matching_mode: current.panoramaMatching,
      });
  ...
  const renderPanoramaFinal = async (): Promise<void> => {
    ...
    try {
      await request(`api/panoramas/${current.panoramaProjectId}/render`, "POST", {
        name: current.panoramaName,
