...
export interface PanoramaViewDependencies {
  PANORAMA_MATCHING_MODES: typeof import("../core/constants").PANORAMA_MATCHING_MODES;
  PANORAMA_PROJECTIONS: typeof import("../core/constants").PANORAMA_PROJECTIONS;
  ...
  panoramaStatusText: typeof import("../tools/panorama-helpers").panoramaStatusText;
...
export function PanoramaOverlay(): ComponentChildren {
  const {
    PANORAMA_MATCHING_MODES,
    PANORAMA_PROJECTIONS,
    capitalize,
    closePanoramaWizard,
  ...
  return (
    <section class={"panorama-card"} role={"dialog"} aria-modal={"true"} aria-labelledby={"panorama-title"}>
      ...
      <footer class={"panorama-footer"}>
        <div class={"panorama-status"}>
          {operationRunning ? (
            <>
              <span>{panoramaStatusText(project)}</span>
              <progress max={progressTotal} value={progressValue} />
            </>
          ) : (
